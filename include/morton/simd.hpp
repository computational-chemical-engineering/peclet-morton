...
///
/// Unlike the auto-vectorised loops in @ref batch.hpp, these are hand-written with AVX-512
/// intrinsics
...
/// unit does *not* need `-mavx512f` and the rest of the binary stays portable. `batch.hpp`
/// dispatches to them at runtime only when `detail::cpu_has_avx512f()` is true, so a single binary
/// uses AVX-512 on a capable CPU and the scalar/AVX2 path elsewhere. Codes that are not 64 bits
/// wide keep the scalar path. Validate changes under Intel SDE (`sde64 -skx`) — most dev/CI hosts
/// lack AVX-512.
///
...
MORTON_AVX512_TARGET inline __m512i compact3(__m512i x) {
  ...
  x = _mm512_and_si512(_mm512_or_si512(x, _mm512_srli_epi64(x, 16)),
                       _mm512_set1_epi64(0x1F00000000FFFFLL));
  x = _mm512_and_si512(_mm512_or_si512(x, _mm512_srli_epi64(x, 32)), _mm512_set1_epi64(0x1FFFFFLL));
  return x;
}
...
MORTON_AVX512_TARGET inline void encode2(const std::uint32_t* x, const std::uint32_t* y,
  ...
  for (; i < n; ++i)
    out[i] = Morton<2, 32>::encode(x[i], y[i]).code();
}
...
MORTON_AVX512_TARGET inline void encode3(const std::uint32_t* x, const std::uint32_t* y,
  ...
  for (; i + 8 <= n; i += 8) {
    ...
    __m512i code = _mm512_or_si512(_mm512_or_si512(spread3(vx), _mm512_slli_epi64(spread3(vy), 1)),
                                   _mm512_slli_epi64(spread3(vz), 2));
    _mm512_storeu_si512((__m512i*)(out + i), code);
  }
  for (; i < n; ++i)
    out[i] = Morton<3, 21>::encode(x[i], y[i], z[i]).code();
}
...
MORTON_AVX512_TARGET inline void add64(const std::uint64_t* in, std::uint64_t* out, std::size_t n,
                                       std::uint64_t notM, std::uint64_t dk, std::uint64_t mask,
                                       std::uint64_t keep) {
  ...
  for (; i < n; ++i)
    out[i] = (((in[i] | notM) + dk) & mask) | (in[i] & keep);
}

MORTON_AVX512_TARGET inline void sub64(const std::uint64_t* in, std::uint64_t* out, std::size_t n,
                                       std::uint64_t dk, std::uint64_t mask, std::uint64_t keep) {
  const __m512i vdk = _mm512_set1_epi64((long long)dk);
  const __m512i vmask = _mm512_set1_epi64((long long)mask);
  const __m512i vkeep = _mm512_set1_epi64((long long)keep);
  std::size_t i = 0;
  ...
  for (; i < n; ++i)
    out[i] = (((in[i] & mask) - dk) & mask) | (in[i] & keep);
}
