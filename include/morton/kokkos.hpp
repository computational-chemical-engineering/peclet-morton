...
template <class T>
using host_ptr_view =
    Kokkos::View<T*, Kokkos::HostSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged>>;
}  // namespace detail
...
void add(const ViewIn& in, const ViewOut& out, unsigned axis, long long delta) {
    ...
    const C mag = neg ? C(static_cast<unsigned long long>(-delta))
                      : C(static_cast<unsigned long long>(delta));
    Kokkos::parallel_for(
        "morton::kokkos::add", Kokkos::RangePolicy<exec>(0, n),
        KOKKOS_LAMBDA(const std::size_t i) {
